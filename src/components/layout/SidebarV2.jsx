...
export const SidebarV2 = ({ mobileOpen, setMobileOpen }) => {
  ...
  const navItems = [
    ...
    { name: "Custom Plan", path: "/custom-plan", icon: FolderKanban },
    { name: "Workout Plans", path: "/plans", icon: ClipboardList },
    ...
    <>
      ...
      <motion.aside
        ...
        className={cn(
          ...
          // Light Mode
          "bg-white/80 border-[#0F172A]/8 backdrop-blur-xl text-[#0F172A]",
          // Dark Mode
          "dark:bg-[#0B0F19]/80 dark:border-white/8 dark:backdrop-blur-xl dark:text-white"
        )}
      >
        {/* Top Branding */}
          ...
          <div className="flex items-center gap-4">
            <div className="min-w-[48px] w-12 h-12 flex items-center justify-center">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center shadow-lg shadow-primary/20">
                <Activity size={20} strokeWidth={2.5} className="text-white animate-heartbeat" />
              </div>
            </div>
        ...
        <div className="p-4 flex-shrink-0 border-t border-black/5 dark:border-white/5">
          <button
            ...
            className="w-full flex items-center gap-4 px-2.5 py-2.5 rounded-2xl text-[#64748B] dark:text-[#94A3B8] hover:bg-red-500/5 hover:text-danger dark:hover:text-danger transition-all overflow-hidden group border border-transparent cursor-pointer"
          >
      ...
      {/* MOBILE OVERLAY MENU (SLIDE OUT DRAWER) */}
      <AnimatePresence>
        {mobileOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex">
            ...
            <motion.div
              ...
              className={cn(
                "relative z-10 w-72 h-full flex flex-col border-r border-[#0F172A]/10 dark:border-white/10",
                "bg-white/95 dark:bg-[#0B0F19]/95 backdrop-blur-xl text-[#0F172A] dark:text-white"
              )}
            >
              <div className="h-20 px-6 flex justify-between items-center border-b border-black/5 dark:border-white/5">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center">
                    <Activity size={18} className="text-white animate-heartbeat" />
                  </div>
                  <h2 className="font-heading font-bold text-md tracking-tight bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Vokey Fitness AI</h2>
                </div>
                <button
                  onClick={() => setMobileOpen(false)}
              ...
              <div className="p-4 border-t border-black/5 dark:border-white/5">
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-4 px-4 py-3 rounded-2xl text-[#64748B] dark:text-[#94A3B8] hover:bg-red-500/5 hover:text-danger transition-all cursor-pointer"
                >
                  <LogOut size={18} />
